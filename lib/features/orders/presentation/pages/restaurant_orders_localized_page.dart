...
class _RestaurantOrdersPageState extends State<RestaurantOrdersPage>
  ...
  static const List<_OrderFilterItem> _filters = <_OrderFilterItem>[
    _OrderFilterItem(code: 'ALL', ru: 'Все', kk: 'Барлығы'),
    _OrderFilterItem(code: 'CREATED', ru: 'Новые', kk: 'Жаңа'),
    _OrderFilterItem(code: 'ACCEPTED', ru: 'Приняты', kk: 'Қабылданды'),
    _OrderFilterItem(code: 'COOKING', ru: 'Готовятся', kk: 'Дайындалуда'),
    _OrderFilterItem(code: 'READY', ru: 'Готовы', kk: 'Дайын'),
    _OrderFilterItem(code: 'ON_THE_WAY', ru: 'В пути', kk: 'Жолда'),
    _OrderFilterItem(code: 'DELIVERED', ru: 'Доставлены', kk: 'Жеткізілді'),
    _OrderFilterItem(code: 'REJECTED', ru: 'Отклонены', kk: 'Қабылданбады'),
    _OrderFilterItem(code: 'CANCELED', ru: 'Отменены', kk: 'Бас тартылды'),
  ];
  ...
  Future<void> _loadOrders({bool silent = false}) async {
    ...
    try {
      ...
      if (!mounted || generation != _loadGeneration) return;
      if (silent) {
        debugPrint('Restaurant orders silent refresh failed: $error');
        return;
      }
      setState(() {
        _error = _safeError(
          ...
          'Тапсырыстарды жүктеу мүмкін болмады. Интернетті тексеріп, қайталаңыз.',
        );
        _isLoading = false;
      });
    } finally {
  ...
  Future<void> _refresh() => _loadOrders(silent: true);
  ...
  void _applyFilter() {
    ...
    _visibleOrders = _allOrders
        .where((order) => _status(order) == _selectedStatus)
        .toList();
  }
  ...
  Future<void> _openOrder(Map<String, dynamic> order) async {
    ...
    if (orderId.isEmpty) {
      _showSnackBar(
        _t(
          'Не удалось определить заказ. Обновите список и повторите.',
          'Тапсырысты анықтау мүмкін болмады. Тізімді жаңартып, қайталаңыз.',
        ),
      );
      return;
    }
    ...
    if (mounted) await _loadOrders(silent: true);
  }
    ...
    try {
      ...
      _showSnackBar(
        _safeError(
          ...
          'Тапсырысты жаңарту мүмкін болмады. Қайта көріңіз.',
        ),
      );
    } finally {
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) {
          ...
          return AlertDialog(
            ...
            title: Text(
              _t('Отклонить заказ?', 'Тапсырысты қабылдамау керек пе?'),
              ...
              children: [
                Text(
                  _t(
                    ...
                    'Қабылдамау себебін көрсетіңіз. Ол тапсырыс тарихында сақталады.',
                  ),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: _t(
                      'Например: блюдо закончилось',
                      'Мысалы: тағам таусылды',
                    ),
            ...
            actions: [
              ...
              FilledButton(
                ...
                onPressed: canSubmit
                    ...
                    : null,
                child: Text(_t('Отклонить', 'Қабылдамау')),
              ),
            ],
          );
        },
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (dialogContext, setDialogState) {
          ...
          return AlertDialog(
            backgroundColor: const Color(0xFF111827),
            title: Text(
              _t('Отменить заказ?', 'Тапсырыстан бас тарту керек пе?'),
              style: const TextStyle(color: Colors.white),
            ),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              ...
              children: [
                Text(
                  paid
                      ? _t(
                          ...
                          'Тапсырыс төленген. Бас тартқаннан кейін JETKIZ клиентке төлемді қайтаруды бастайды. Себебін көрсетіңіз.',
                        )
                      : _t(
                          'Отмена завершит заказ. Укажите причину — она сохранится в истории.',
                          'Бас тарту тапсырысты аяқтайды. Себебін көрсетіңіз — ол тарихта сақталады.',
                        ),
                  style: const TextStyle(color: Color(0xFFCBD5E1)),
                ),
                const SizedBox(height: 14),
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: _t(
                      'Например: закончился ингредиент',
                      'Мысалы: ингредиент таусылды',
                    ),
                    hintStyle: const TextStyle(color: Color(0xFF64748B)),
                    filled: true,
                    fillColor: const Color(0xFF0B1220),
            ...
            actions: [
              TextButton(
                onPressed: () => Navigator.of(dialogContext).pop(),
                child: Text(_t('Не отменять', 'Бас тартпау')),
              ),
              FilledButton(
                style: FilledButton.styleFrom(
                  backgroundColor: const Color(0xFFDC2626),
                ...
                onPressed: canSubmit
                    ? () => Navigator.of(dialogContext).pop(controller.text.trim())
                    : null,
                child: Text(_t('Отменить заказ', 'Тапсырыстан бас тарту')),
              ),
            ],
          );
        },
  ...
  String _statusChangedMessage(String status, {required bool isPickup}) {
    switch (status) {
      case 'ACCEPTED':
        return _t('Заказ принят', 'Тапсырыс қабылданды');
      case 'COOKING':
        return _t(
          'Заказ переведён в приготовление',
          'Тапсырыс дайындалуға жіберілді',
        );
      case 'READY':
        return isPickup
            ? _t('Заказ готов к выдаче', 'Тапсырыс беруге дайын')
            : _t('Заказ готов', 'Тапсырыс дайын');
      case 'CANCELED':
        return _t('Заказ отменён', 'Тапсырыстан бас тартылды');
      case 'REJECTED':
        return _t('Заказ отклонён', 'Тапсырыс қабылданбады');
      default:
        return _t('Статус заказа обновлён', 'Тапсырыс мәртебесі жаңартылды');
    }
  }

  String _status(Map<String, dynamic> order) =>
      _string(order['status']).toUpperCase();

  String _fulfillmentType(Map<String, dynamic> order) =>
      _string(order['fulfillmentType']).toUpperCase();

  bool _isPickup(Map<String, dynamic> order) =>
      _fulfillmentType(order) == 'PICKUP';

  bool _isPickupVerified(Map<String, dynamic> order) =>
      _dateTime(order['pickupCodeVerifiedAt']) != null;

  bool _isIssuedPickup(Map<String, dynamic> order) =>
      _isPickup(order) &&
      (_isPickupVerified(order) || _status(order) == 'DELIVERED');

  String _orderId(Map<String, dynamic> order) => _string(order['id']);

  String _orderNumber(Map<String, dynamic> order) {
    final text = order['number']?.toString().trim() ?? '';
    return text.isEmpty || text.toLowerCase() == 'null' ? '—' : '#$text';
  }

  int _money(Map<String, dynamic> order, String key) => _int(order[key]);

  int _itemsCount(Map<String, dynamic> order) {
    final count = _int(order['itemsCount']);
    if (count > 0) return count;
    final items = order['items'];
    if (items is List) return items.length;
    final previewItems = order['previewItems'];
    return previewItems is List ? previewItems.length : 0;
  }
  ...
  List<_OrderPreviewItem> _previewItems(Map<String, dynamic> order) {
    ...
    return raw.whereType<Map>().map((item) {
      ...
      return _OrderPreviewItem(
        title: title.isEmpty ? _t('Позиция', 'Тауар') : title,
  ...
  String _clientName(Map<String, dynamic> order) {
    ...
    if (user is Map) {
      final fullName =
          '${_string(user['firstName'])} ${_string(user['lastName'])}'.trim();
  ...
  bool _isUpdating(Map<String, dynamic> order) {
    ...
    return _updatingOrderIds.contains(id) || _pendingReadyOrderIds.contains(id);
  }
  ...
  List<_OrderAction> _actionsFor(Map<String, dynamic> order) {
    ...
    switch (status) {
      case 'CREATED':
        return <_OrderAction>[
          _OrderAction(
            label: _t('Принять', 'Қабылдау'),
            nextStatus: 'ACCEPTED',
            icon: Icons.check_circle_outline,
            isPrimary: true,
          ),
          _OrderAction(
            label: _t('Отклонить', 'Қабылдамау'),
            nextStatus: 'REJECTED',
      ...
      case 'ACCEPTED':
        return <_OrderAction>[
          _OrderAction(
            label: _t('Готовить', 'Дайындау'),
            nextStatus: 'COOKING',
            icon: Icons.restaurant_rounded,
            isPrimary: true,
          ),
          _OrderAction(
            label: _t('Отменить', 'Бас тарту'),
            nextStatus: 'CANCELED',
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _RestaurantOrderCard(
          ...
          courierName: pickup
              ? _t('Клиент заберёт сам', 'Клиент өзі алып кетеді')
              : _courierName(order),
          ...
          onActionTap: (action) {
            ...
            } else if (action.nextStatus == 'CANCELED') {
              unawaited(_confirmCancel(order));
            } else if (action.nextStatus == 'READY') {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        final borderAlpha =
            isNew ? 0.35 + (blinkController.value * 0.35) : 0.16;
        return Material(
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  orderNumber == '—'
                      ? context.tr('Заказ', 'Тапсырыс')
                      : '${context.tr('Заказ', 'Тапсырыс')} $orderNumber',
                  maxLines: 1,
            ...
            children: [
              Expanded(
                child: _SmallInfoBlock(
                  label: context.tr('Клиент', 'Клиент'),
                  value: clientName,
              ...
              Expanded(
                child: _SmallInfoBlock(
                  label: isPickup
                      ? context.tr('Получение', 'Алу')
                      : context.tr('Курьер', 'Курьер'),
                  value: courierName,
                  icon: isPickup
          ...
          Row(
            children: [
              Expanded(
                child: _SmallInfoBlock(
                  label: context.tr('Создан', 'Құрылды'),
                  value: createdAt,
              ...
              Expanded(
                child: _SmallInfoBlock(
                  label: context.tr('Обещано к', 'Дайын болу уақыты'),
                  value: promisedAt,
                ...
                return Expanded(
                  child: Padding(
                    padding: EdgeInsets.only(
                      right: identical(action, actions.last) ? 0 : 8,
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          const Icon(Icons.storefront_rounded, size: 13, color: color),
...
class _MoneyRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          _Line(label: context.tr('Подытог', 'Аралық сома'), value: '$subtotal ₸'),
          const SizedBox(height: 5),
          _Line(
            label: context.tr('Доставка', 'Жеткізу'),
            value: isPickup
                ? context.tr('Самовывоз', 'Өзі алып кету')
                : '$deliveryFee ₸',
          ),
          ...
          _Line(
            ...
            value: _paymentStatusLabel(context, paymentStatus),
          ),
          const Divider(height: 16, color: Color(0xFF1F2937)),
          _Line(
            label: context.tr('Итого', 'Барлығы'),
            value: '$total ₸',
            strong: true,
          ),
        ],
  ...
  String _paymentStatusLabel(BuildContext context, String status) {
    switch (status.trim().toUpperCase()) {
      case 'PAID':
        return context.tr('Оплачено', 'Төленді');
      case 'PENDING':
        return context.tr('Ожидает оплаты', 'Төлем күтілуде');
      case 'FAILED':
        return context.tr('Ошибка оплаты', 'Төлем орындалмады');
      case 'REFUNDED':
        return context.tr('Возврат', 'Қайтарылды');
      case 'CANCELED':
      case 'CANCELLED':
        return context.tr('Отменено', 'Бас тартылды');
      default:
        return context.tr('Не указано', 'Көрсетілмеген');
    }
  }
}
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  itemsCount > 0
                      ? '${context.tr('Позиции заказа', 'Тапсырыс тауарлары')}: $itemsCount'
                      : context.tr('Позиции заказа', 'Тапсырыс тауарлары'),
                  style: const TextStyle(
                    color: Colors.white,
                    fontSize: 13,
                    fontWeight: FontWeight.w800,
          ...
          if (visibleItems.isEmpty)
            Text(
              context.tr('Позиции не указаны', 'Тауарлар көрсетілмеген'),
              style: const TextStyle(
                color: Color(0xFF94A3B8),
                fontSize: 12,
              ),
          ...
          if (items.length > 3)
            Text(
              '${context.tr('Ещё', 'Тағы')} ${items.length - 3}',
              style: const TextStyle(
                color: Color(0xFF64748B),
                fontSize: 11,
                fontWeight: FontWeight.w600,
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: Text(
          isLoading ? context.tr('Обновление...', 'Жаңартылуда...') : action.label,
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        Text(
          all
              ? context.tr('Заказов пока нет', 'Әзірге тапсырыс жоқ')
              : context.tr(
                  'Заказов с выбранным статусом нет',
                  'Таңдалған мәртебеде тапсырыс жоқ',
                ),
          textAlign: TextAlign.center,
          style: const TextStyle(
            color: Colors.white,
        ...
        Text(
          all
              ? context.tr(
                  ...
                  'Жаңа тапсырыстар осы экранда пайда болады',
                )
              : context.tr(
...
class _OrderFilterItem {
  ...
  String label(BuildContext context) => context.tr(ru, kk);
}
    ...
    switch (status.trim().toUpperCase()) {
      case 'CREATED':
        return _OrderStatusMeta(
          ...
          backgroundColor: const Color(0x1A60A5FA),
          borderColor: const Color(0x3360A5FA),
        );
      case 'ACCEPTED':
        return _OrderStatusMeta(
